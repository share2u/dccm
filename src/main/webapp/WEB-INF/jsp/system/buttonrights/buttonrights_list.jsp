...
<body class="no-skin">
						...
						<div class="col-xs-12">
							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="margin-top:7px;">
								...
								<tr>
									...
									<c:if test="${QX.edit == 1 }">
										<!-- 按钮循环 -->
										<c:forEach items="${buttonlist}" var="ncutbutton" varStatus="vsb">
											<th class='center'>${ncutbutton.NAME }</th>
										</c:forEach>
									</c:if>
								</tr>
								</thead>
										...
										<c:if test="${QX.edit == 1 }">
											...
											<c:forEach items="${buttonlist}" var="ncutbutton" varStatus="vsb">
												...
												<c:forEach items="${roleFhbuttonlist}" var="varRb" varStatus="vsRb">
													<c:if test="${var.ROLE_ID == varRb.ROLE_ID && ncutbutton.NCUTBUTTON_ID == varRb.BUTTON_ID }">
														<c:set value="1" var="rbvalue"></c:set>
													</c:if>
												</c:forEach>
												<td class='center' style="height: 20px;">
													<label>
														<input name="switch-field-1" onclick="upRb('${var.ROLE_ID}','${ncutbutton.NCUTBUTTON_ID}')" class="ace ace-switch ace-switch-3" type="checkbox" <c:if test="${rbvalue == 1 }">checked="checked"</c:if> >
														<span class="lbl"></span>
													</label>
												</td>
							...
							<table style="width:100%;">
								<tr>
									<td style="vertical-align:top;">
										<c:if test="${QX.cha == 1 }">
										<a class="btn btn-mini btn-success" onclick="window.location.href='<%=basePath%>buttonrights/list.do?type=2';">切换视图</a>
										</c:if>
									</td>
								</tr>
							</table>
	...
	<script type="text/javascript">
		...
		//处理按钮点击
		function upRb(ROLE_ID,NCUTBUTTON_ID){
			$.ajax({
				type: "POST",
				url: "<%=basePath%>buttonrights/upRb.do?ROLE_ID="+ROLE_ID+"&BUTTON_ID="+NCUTBUTTON_ID+"&guid="+new Date().getTime(),
		    	data: encodeURI(""),
				dataType:'json',
