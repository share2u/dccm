		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					...
					<form action="ordermx/${msg }.do" name="Form" id="Form" method="post">
						<input type="hidden" name="ORDERMX_ID" id="ORDERMX_ID" value="${pd.ORDERMX_ID}"/>
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							...
							<tr>
								...
								<td>
									<c:if test="${pd.PAY_METHOD==0}">微信支付</c:if>
									<c:if test="${pd.PAY_METHOD==1}">支付宝支付</c:if>
									<c:if test="${pd.PAY_METHOD==2}">储值卡支付</c:if>
									<c:if test="${pd.PAY_METHOD==3}">现金支付</c:if>
									<c:if test="${pd.PAY_METHOD==4}">银行卡支付</c:if>
								</td>
							</tr>
							<tr>
