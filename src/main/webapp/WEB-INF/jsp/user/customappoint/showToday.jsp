...
<body class="no-skin">
				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="customappoint/showToday.do" method="post"
								name="Form" id="Form">
								<table style="margin-top:5px;">
									<tr>
										<td>
											<div class="nav-search">
												<span class="input-icon"> <input type="text"
													placeholder="验证码/手机号/备注" class="nav-search-input"
										...
										<td style="vertical-align:top;padding-left:2px;"><b>请选择门店：</b>
											<select class="chosen-select form-control" name="STOREID"
											id="STOREID" data-placeholder="请选择门店"
										...
										<c:choose>
											...
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.CUSTOMAPPOINT_ID}" class="ace" /><span class="lbl"></span></label>
															...
															<td class='center'>${var.ORDER_MONEY}</td>
															...
															<td class="center"><c:if
																...
																<div class="hidden-sm hidden-xs btn-group">
																	<c:if test="${QX.edit == 1 }">
																		<a class="btn btn-xs btn-success" title="修改预约时间"
																			onclick="edit('${var.CUSTOMAPPOINT_ID}');"> <i
																			class="ace-icon fa fa-pencil-square-o bigger-120"
																			title="修改预约时间"></i> </a>
																	</c:if>

																	<a class="btn btn-light btn-xs" title="打印订单"
																		onclick="printOrder('${var.ORDER_ID}');"> <i
																		class="ace-icon fa fa-download bigger-110 nav-search-icon blue"
																		title="打印订单"></i> </a> <a class="btn btn-xs btn-warning"
																		onclick="endorder('${var.CUSTOMAPPOINT_ID}');"> <i
																		class="ace-icon fa fa-legal bigger-120" title="到店"></i>
																	</a>
																		...
																		<ul
																			...
																			<c:if test="${QX.edit == 1 }">
																				<li><a style="cursor:pointer;"
																					onclick="printOrder('${var.ORDER_ID}');"
																			...
																			<c:if test="${QX.del == 1 }">
																				<li><a style="cursor:pointer;"
																					onclick="del('${var.CUSTOMAPPOINT_ID}');"
	...
	//确认完成
		function endorder(Id){
			$.ajax({
				type:"POST",
				url:"customappoint/judgeStatus.do",
				dataType : "json",
				data:{
					CUSTOMAPPOINT_ID:Id,
				},
				success:function(data){
					if(data.result=="false"){
						alert("该订单的订单状态错误，不能确认治疗！");
						window.location.reload();
						return;
						
					}else {
						bootbox.confirm("您确认客户已经完成治疗吗?", function(result) {
						if(result) {
							top.jzts();
							var url = "<%=basePath%>customappoint/endorder.do?CUSTOMAPPOINT_ID="+Id+"&tm="+new Date().getTime();
							$.get(url,function(data){
								//nextPage(${page.currentPage});
							//window.location.href='<%=basePath%>customappoint/showToday.do';
							window.location.reload();
						});
					}
					});
			...
			bootbox.confirm(msg, function(result) {
				if(result) {
					...
					if(str==''){
						...
						if(msg == '您确认选中的客户都已经完成治疗吗?'){
							top.jzts();
							$.ajax({
								type: "POST",
								url: '<%=basePath%>customappoint/endorderAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		function printOrder(ORDER_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="打印订单";
			 diag.URL = '<%=basePath%>customappoint/goPrintOrder.do?ORDER_ID='+ORDER_ID;
			 diag.Width = 900;
			 diag.Height = 650;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
