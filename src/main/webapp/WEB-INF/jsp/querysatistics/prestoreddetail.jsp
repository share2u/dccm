			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="queryMember/prestoredDetail.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<!--<td class='center'>${var.BANK}</td> -->
											<%-- <td class='center'><c:if test="${var.ISSCAN==0}">可以使用</c:if>
												   <c:if test="${var.ISSCAN==1}">不可使用</c:if></td> --%>
										<%--	<td class='center'>${var.STORENAME}</td> --%>
											<td align='right'>
											  <fmt:formatNumber type="number" value="${var.PRESTOREMONEY}" pattern="0.00" maxFractionDigits="2"/>
											</td>
											<td class='center'>${var.STAFF_NAME}</td>
											<td class='center'>${var.CREATE_TIME}</td>
					
											<td class='center'><c:if test="${var.TYPE==1}">收入</c:if>
												    <c:if test="${var.TYPE==2}">支出</c:if>
												    <c:if test="${var.TYPE==3}">退款</c:if>
											</td>
											<%-- <td class="center">
