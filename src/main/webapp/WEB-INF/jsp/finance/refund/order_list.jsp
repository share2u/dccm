...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="order/refund.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="姓名/手机号" class="nav-search-input" id="nav-search-input" autocomplete="off" name="keywords" value="${pd.keywords }" placeholder="这里输入关键词"/>
										...
										<tr> 
												...
												<c:if test="${var.ORDER_STATUS==1}"><font color="red">已关闭</font></c:if>
												 <c:if test="${var.ORDER_STATUS==2}"><font color="green">已预约</font></c:if>
												  ...
												  <c:if test="${var.ORDER_STATUS==4}"><font color="green">待评价</font></c:if>
												   <c:if test="${var.ORDER_STATUS==5}">已完成</c:if>
											</td>	
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="查看明细" onclick="edit('${var.ORDER_ID}');">
														退费
													</a>
													</c:if>
													<!--<c:if test="${QX.del == 1 }">
						...
						<table style="width:100%;">
							<tr>
							 	 <td style="vertical-align:top;">
									...
									<%-- <c:if test="${QX.del == 1 }">
									<a class="btn btn-mini btn-success" onclick="makeAll('确定要给选中的订单退费么?');" title="批量退费" >批量退费</a>
									</c:if> --%> 
								</td> 
	...
	<script type="text/javascript">
		...
		//删除
		function del(Id){
			bootbox.confirm("确定要退费吗?", function(result) {
				if(result) {
					top.jzts();
					...
					$.get(url,function(data){
						if("success" == data.result){
							...
							top.hangge();
							bootbox.dialog({
								message: "<span class='bigger-110'>退费失败</span>",
								buttons: 			
								{
									"button" :
									{
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="取消订单";
			 diag.URL = '<%=basePath%>order/goEditRefund.do?ORDER_ID='+Id;
			 diag.Width = 800;
			 diag.Height = 700;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function makeAll(msg){
			bootbox.confirm(msg, function(result) {
				if(result) {
					...
					console.log(document.getElementsByName('ids')[0].value);
					if(str==''){
						bootbox.dialog({
						...
						if(msg == '确定要给选中的订单退费么?'){
							...
							var diag = new top.Dialog();
							 diag.Drag=true;
							 diag.Title ="取消订单";
							 diag.URL = '<%=basePath%>order/deleteAll.do?ORDER_ID='+str;
							 diag.Width = 800;
							 diag.Height = 700;
							 diag.Modal = true;				//有无遮罩窗口
							 diag. ShowMaxButton = true;	//最大化按钮
