package cn.ncut.controller.system.login;
...
import cn.ncut.util.RightsHelper;
import cn.ncut.util.Tools;
/**
 * 
*
*<p>Title:LoginController</p>
*<p>Description:系统总入口</p>
*<p>Company:ncut</p> 
* @author lph 
* @date 2016-12-5下午8:31:16
*
 */
@Controller
public class LoginController extends BaseController {
	...
	@Resource(name="ncutbuttonService")
	private NcutbuttonManager fhbuttonService;
	...
	public Object login()throws Exception{
		...
		pd = this.getPageData();
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").replaceAll("ncut1110", "").replaceAll("dccm", "").split(",ncut,");
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					pd = staffService.getStaffByNameAndPwd(pd);	//根据用户名和密码去读取用户信息
					if(pd != null){
						pd.put("LAST_LOGIN",DateUtil.getTime().toString());
						staffService.updateLastLogin(pd);
						Staff staff = new Staff();
						staff.setSTAFF_ID(pd.getString("STAFF_ID"));
						staff.setSTORE_ID(pd.getString("STORE_ID"));
						staff.setUSERNAME(pd.getString("USERNAME"));
						staff.setPASSWORD(pd.getString("PASSWORD"));
						staff.setSTAFF_NAME("STAFF_NAME");
						staff.setRIGHTS(pd.getString("RIGHTS"));
						staff.setROLE_ID(pd.getString("ROLE_ID"));
						staff.setLAST_LOGIN(pd.getString("LAST_LOGIN"));
						staff.setIP(pd.getString("IP"));
						session.setAttribute(Const.SESSION_USER, staff);			//把用户信息放session中
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu){
		...
		try{
			Session session = Jurisdiction.getSession();
			Staff staff = (Staff)session.getAttribute(Const.SESSION_USER);						//读取session中的用户信息(单独用户信息)
			if (staff != null) {
				Staff stafff = (Staff)session.getAttribute(Const.SESSION_USERROL);				//读取session中的用户信息(含角色信息)
				if(null == stafff){
					staff = staffService.getStaffAndRoleById(staff.getSTAFF_ID());				//通过用户ID读取用户信息和角色信息
					session.setAttribute(Const.SESSION_USERROL, staff);						//存入session	
				}else{
					staff = stafff;
				}
				String USERNAME = staff.getUSERNAME();
				Role role = staff.getRole();													//获取用户角色
	...
	public ModelAndView defaultPage() throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
	//	pd.put("staffCount", Integer.parseInt(staffService.getStaffCount("").get("staffCount").toString())-1);				//系统用户数
	//	pd.put("staffCount", Integer.parseInt(staffService.getStaffCount("").get("staffCount").toString()));	//会员数
		mv.addObject("pd",pd);
	...
	public PageData setLoginPd(PageData pd){
		...
		if(null != strLOGINEDIT && !"".equals(strLOGINEDIT)){
			String strLo[] = strLOGINEDIT.split(",ncut,");
			if(strLo.length == 2){
	...
	public Map<String, String> getUQX(String USERNAME){
		...
		try {
			...
			pd.put("ROLE_ID", staffService.findByUsername(pd).get("ROLE_ID").toString());//获取角色ID
