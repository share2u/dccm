package cn.ncut.controller.system.role;
...
public class RoleController extends BaseController {
	...
	public ModelAndView add()throws Exception{
		...
		try{
			...
			//暂时注销日志，加staff需要改
			//NcutLOG.save(Jurisdiction.getUsername(), "新增角色:"+pd.getString("ROLE_NAME"));
		} catch(Exception e){
			logger.error(e.toString(), e);
	...
	public ModelAndView edit()throws Exception{
		...
		try{
			...
			roleService.edit(pd);
			//暂时注销日志，加staff需要改
			//NcutLOG.save(Jurisdiction.getUsername(), "修改角色:"+pd.getString("ROLE_NAME"));
	...
	public Object deleteRole(@RequestParam String ROLE_ID)throws Exception{
		...
		try{
			...
			//	List<PageData> appuserlist = staffService.listAllStaffByRorlid(pd);	//此角色下的会员
				if(userlist.size() > 0){						//此角色已被使用就不能删除
					errInfo = "false2";
				}else{
				roleService.deleteRoleById(ROLE_ID);	//执行删除
				//暂时注销日志，加staff需要改
				//NcutLOG.save(Jurisdiction.getUsername(), "删除角色ID为:"+ROLE_ID);
				errInfo = "success";
				}
			}
		} catch(Exception e){
	...
	public void saveMenuqx(@RequestParam String ROLE_ID,@RequestParam String menuIds,PrintWriter out)throws Exception{
		...
		//暂时注销日志，加staff需要改
		//NcutLOG.save(Jurisdiction.getUsername(), "修改角色菜单权限，角色ID为:"+ROLE_ID);
		PageData pd = new PageData();
		try{
	...
	public void saveB4Button(@RequestParam String ROLE_ID,@RequestParam String menuIds,@RequestParam String msg,PrintWriter out)throws Exception{
		...
		//暂时注销日志，加staff需要改
		//NcutLOG.save(Jurisdiction.getUsername(), "修改"+msg+"权限，角色ID为:"+ROLE_ID);
		PageData pd = new PageData();
		pd = this.getPageData();
		try{
			if(null != menuIds && !"".equals(menuIds.trim())){
