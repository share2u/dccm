...
import cn.ncut.entity.wechat.message.resp.Article;
import cn.ncut.entity.wechat.message.resp.Music;
import cn.ncut.entity.wechat.pojo.SNSUserInfo;
import cn.ncut.entity.wechat.pojo.WeixinGroup;
import cn.ncut.entity.wechat.pojo.WeixinMedia;
import cn.ncut.entity.wechat.pojo.WeixinOauth2Token;
import cn.ncut.entity.wechat.pojo.WeixinQRCode;
import cn.ncut.entity.wechat.pojo.WeixinUserInfo;
import cn.ncut.entity.wechat.pojo.WeixinUserList;
...
public class AdvancedUtil {
	...
	public static SNSUserInfo getSNSUserInfo(String accessToken, String openId) {
		...
		if (null != jsonObject) {
			try {
				...
				// 昵称
				snsUserInfo.setNickname(EmojiUtil.emojiChange(jsonObject.getString("nickname")));
				// 性别（1是男性，2是女性，0是未知）
	...
	public static void main(String args[]) throws IOException {
		...
		AdvancedUtil mm = new AdvancedUtil();
		info.load(mm.getClass().getClassLoader()
				.getResourceAsStream("mian.properties"));

		// 第三方用户唯一凭证
		String appId = info.getProperty("appId");
		// 第三方用户唯一凭证密钥
		String appSecret = info.getProperty("appSecret");
		// 获取接口访问凭证
		 ...
		 */
		/*
		 * // 组装文本客服消息 String jsonTextMsg =
		 * makeTextCustomMessage("oQOt1w6tHG2rmsCci8rxbpB5neh4",
		 * "点击查看<a href=\"http://blog.csdn.net/lyq8479\">柳峰的博客</a>"); // 发送客服消息
		 ...
		 * list.add(article2); // 组装图文客服消息 String jsonNewsMsg =
		 * makeNewsCustomMessage("oEdzejiHCDqafJbz4WNJtWTMbDcE", list); //
		 * 发送客服消息 sendCustomMessage(accessToken, jsonNewsMsg);
		 */
		/**
		 * 创建临时二维码
		 */
		 ...
		 * 111111); // 临时二维码的ticket
		 * System.out.println(weixinQRCode.getTicket()); // 临时二维码的有效时间
		...
		/*
		 * String ticket =
		 * "gQEg7zoAAAAAAAAAASxodHRwOi8vd2VpeGluLnFxLmNvbS9xL2lIVVJ3VmJsTzFsQ0ZuQ0Y1bG5WAAIEW35+UgMEAAAAAA=="
		 * ; String savePath = "G:/download"; // 根据ticket换取二维码 getQRCode(ticket,
		 * savePath);
		 */
		/**
		 * 获取用户信息
		 ...
		 */
		/*
		 * List<WeixinGroup> groupList = getGroups(accessToken); // 循环输出各分组信息
