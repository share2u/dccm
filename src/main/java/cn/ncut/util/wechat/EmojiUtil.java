...
public class EmojiUtil {
	...
	public static String emojiChange(String string) {
		...
		try {
			//logger.debug("--------------------------------------");
			//logger.debug("all-string:" + string);
			//logger.debug("all-unicode:" + convert(string));
			Pattern pattern = Pattern.compile(unicodeReg,Pattern.UNICODE_CASE | Pattern.CASE_INSENSITIVE);
			...
			for (int i = 0; i < string.length(); i++) {
				...
				//logger.debug("temp:" + temp + ";unicode:" + convert(temp));
			}
			//logger.debug("sb:" + sbBuffer.toString());
			//logger.debug("--------------------------------------");
